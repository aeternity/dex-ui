...
export default {
  ...
  computed: {
    ...
    footerText() {
      if (this.importing) return this.$t('import.fetchingData');
      if (!this.address) return this.$t('connectWalletFindPool');
      if (!this.tokenA || !this.tokenB) return this.$t('import.selectBothTokens');
      if (this.imported) return this.$t('import.imported');
      return this.$t('import.footerText');
    },
  },
  methods: {
